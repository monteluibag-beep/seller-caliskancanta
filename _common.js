// Ortak JS - Her sayfaya <script type="module"> içinde import edilecek
...
function updateRateUI() {
  ...
  [
    {key:'USD', valId:'r-usd', chgId:'r-usd-c', mobId:'mob-usd'},
    {key:'EUR', valId:'r-eur', chgId:'r-eur-c', mobId:'mob-eur'},
    {key:'GBP', valId:'r-gbp', chgId:'r-gbp-c', mobId:null},
  ].forEach(r => {
    const val = rates[r.key], prev = prevRates[r.key];
    const diff = val - prev, pct = ((diff/prev)*100).toFixed(2);
    const up = diff >= 0, cls = up ? 'up' : 'dn', arrow = up ? '▲' : '▼';
    const ve = document.getElementById(r.valId);
    const ce = document.getElementById(r.chgId);
